...
ClientRouter.get("/cliente", authToken, async(req: Request, res:Response)=> {
    ...
    res.status(200).send("voce esta em /cliente get");
})

ClientRouter.get("/cliente/:id", authToken, async (req: Request, res: Response) =>{
    const idFromUser: string = req.params.id;
    const clientById = await ClientController.getClientById(idFromUser)
})

ClientRouter.post("/cliente", authToken, async(req: Request, res: Response) => {
...
ClientRouter.put ("/cliente/:id", authToken, async (req: Request, res: Response) =>{
    const idFromUser: string = req.params.id;
    const clientData: ICliente = req.body;
    const updatedClient = await ClientController.updateClient(idFromUser, clientData);
    if(updatedClient){
...
ClientRouter.delete("/cliente/:id", authToken, async (req: Request, res: Response) => {
    const idFromUser: string = req.params.id;
